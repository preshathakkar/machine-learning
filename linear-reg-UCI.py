...
parsedSamplePoints = map(lambda a : parsePoint(a), samplePoints)
...
# TODO: Replace <FILL IN> with appropriate code
parsedData = parsedDataInit.map(lambda a: LabeledPoint((a.label - 1922.0),a.features))
...
averageTrainYear = (parsedTrainData
                    .map(lambda a:a.label).reduce(lambda a,b: a+ b) / nTrain)
...
def calcRMSE(labelsAndPreds):
    ...
    mean = labelsAndPreds.map(lambda a : squaredError(a[0],a[1])).reduce(lambda a,b: a+b)
    #print mean
    mse = mean / labelsAndPreds.count()
...
def gradientSummand(weights, lp):
    ...
    """
    mulFact = weights.dot(lp.features) - lp.label
    return mulFact * lp.features
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        gradient = trainData.map(lambda a: gradientSummand(w, a)).reduce(lambda a,b: a+b)
...
labelsAndPreds = parsedValData.map(lambda a :getLabeledPrediction(weightsLR0,a))
...
def twoWayInteractions(lp):
    ...
    """
    twoWay = list()
    twoWay.extend(lp.features)
    for x in lp.features:
        twoWay.extend(x*lp.features)
    return LabeledPoint(lp.label,twoWay)

print twoWayInteractions(LabeledPoint(0.0, [2, 3]))

# Transform the existing train, validation, and test sets to include two-way interactions.
trainDataInteract = parsedTrainData.map(lambda a: twoWayInteractions(a))
valDataInteract = parsedValData.map(lambda a: twoWayInteractions(a))
testDataInteract = parsedTestData.map(lambda a: twoWayInteractions(a))
...
# TODO: Replace <FILL IN> with appropriate code 
labelsAndPredsTest = testDataInteract.map(lambda a : (a.label, modelInteract.predict(a.features)))
