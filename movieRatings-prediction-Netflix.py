...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    RatingCount = len(RatingTuple)
    RatingAvg = sum(x for x in RatingTuple) / float(RatingCount)
    return MovieID, (RatingCount, RatingAvg)
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda x : ( (x[0][0],x[0][1]), math.pow((x[1][0] - x[1][1]), 2) )))

    # Compute the total squared error - do not use collect()
    totalError = squaredErrorsRDD.map(lambda x : x[1]).reduce(lambda a,b : a+b)
...
#myRatedMoviesID = [ seq[0] for seq in myRatedMovies ]
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda x : x[0] not in [ seq[0] for seq in myRatedMovies ]).map(lambda x : (myUserID,x[0])))
#print myUnratedMoviesRDD.take(3)
...
predictedWithCountsRDD  = (predictedRDD
                           .join(movieCountsRDD))
#print predictedWithCountsRDD.take(1)
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .filter(lambda x : x[1][1] >= 75).join(moviesRDD).map(lambda x : (x[1][0][0] , x[1][1])))
