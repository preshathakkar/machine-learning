...
def simpleTokenize(string):
    ...
    noPunct = re.sub(split_regex, ' ',lowerString).split()
    return noPunct
...
def tokenize(string):
    ...
    """
    st = simpleTokenize(string)
    refinedList = [x for x in st if x not in stopwords]
    return refinedList
...
def countTokens(vendorRDD):
    ...
    """
    countMap = vendorRDD.map(lambda rec : rec[1]).reduce(add)
#    print countMap.collect()
    count = len(countMap)
    return count
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.sortBy(lambda x : -len(x[1])).collect()
...
def tf(tokens):
    ...
    for key in tfDict:
        tfDict[key] = float(tfDict[key])/totalTokens
    return tfDict
...
# TODO: Replace <FILL IN> with appropriate code
#corpusRDD = amazonRecToToken.map(lambda rec : (rec[0],rec[1])) + googleRecToToken.map(lambda rec : (rec[0],rec[1]))
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.map(lambda rec : (rec[0],list(set(rec[1]))))
    #print uniqueTokens.take(2)
    
    tokenCountPairTuple = uniqueTokens.flatMap(lambda rec : rec[1]).map(lambda x : (x,1))
    #print tokenCountPairTuple.take(12)
    
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(lambda a,b : a+b)
    #print tokenSumPairTuple.take(12)
    
    #toSend = tokenSumPairTuple.map(lambda tup : (tup[0],float(N)/float(tup[1])))
    #print toSend.take(12)
    return (tokenSumPairTuple.map(lambda tup : (tup[0],float(N)/float(tup[1]))))
...
def norm(a):
    ...
    sumSq = 0.00
    for key in a:
        sumSq = sumSq + (a[key] * a[key])
    rootSq = math.sqrt(sumSq)
    return rootSq
...
avgSimDups = (trueDupsRDD.map(lambda x : x[1][0]).reduce(add)) / trueDupsCount

nonDupsRDD = (sims
              .subtractByKey(goldStandard))
avgSimNon = (nonDupsRDD.map(lambda x: x[1]).reduce(add)) / nonDupsRDD.count()
...
def invert(record):
    ...
    """
    recId = record[0]
    recTokenDict = record[1]
    pairs = list()
    for key in recTokenDict:
        pairs.append((key,recId))
    return (pairs)
...
def fastCosineSimilarity(record):
    ...
    for k in tokens:
        s = s + amazonWeightsBroadcast.value[amazonRec][k] * googleWeightsBroadcast.value[googleRec][k]
    #print "Am",amazonNormsBroadcast.value
